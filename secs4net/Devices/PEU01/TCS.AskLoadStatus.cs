...
namespace Cim.Eap {
    partial class Driver {
        void HandleTCS(AskLoadStatusRequest tx) {
        ...
        void CheckPJDuplicated(AskLoadStatusRequest tx) {
            var s16f20 = EAP.Send(EAP.SecsMessages[16, 19, "GetAllPJ"]);
        ...
        void CheckPJSpace() {
            var s16f22 = EAP.Send(EAP.SecsMessages[16, 21, "GetPJSpace"]);
            if ((ushort)s16f22.SecsItem == 0)
                throw new ScenarioException("ProcessJob Space is 0");
        }

        void CheckCJSpace() {
            var s1f4 = EAP.Send(EAP.SecsMessages[1, 3, "GetCJSpace"]);
