...
namespace Cim.Eap {
    partial class Driver {
        ...
        async Task CheckPJDuplicated(AskLoadStatusRequest tx) {
            using (var s16f20 = await EAP.SendAsync(EAP.SecsMessages[16, 19, "CheckPJDuplicated"], false))
            {
        ...
        async Task CheckPJSpace() {
            using (var s16f22 = await EAP.SendAsync(EAP.SecsMessages[16, 21, "CheckPJSpace"], false))
            {
        ...
        async Task CheckCJSpace() {
            using (var s1f4 = await EAP.SendAsync(EAP.SecsMessages[1, 3, "CheckCJSpace"], false))
            {
        ...
        async Task CheckRecipe(AskLoadStatusRequest tx) {
            using (var s7f20 = await EAP.SendAsync(EAP.SecsMessages[7, 19, "GetRecipeList"], false))
            {
                var recipeList =
